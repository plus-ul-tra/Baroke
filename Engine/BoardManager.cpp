...
/*			jokerMagnetic->m_jokerInfo.lifeSpan--;*/
		...
		{
			int count = (boardManager.WhiteStoneRemoveCheck(position) * jokerWaxseal->m_jokerInfo.functionVariable);
			boardManager.m_playerInfo.m_waxMoney += count;
			boardManager.m_playerInfo.m_money -= count; // 흰 돌 제거
			// 일단은 흰돌도 줌

			std::cout << "Wax Money : " << boardManager.m_playerInfo.m_waxMoney << std::endl;
			std::cout << "Money : " << boardManager.m_playerInfo.m_money << std::endl;
		}
		},
...
// 			jokerLight->m_jokerInfo.lifeSpan--;
		...
		{ StoneAbility::jokerTime, [this](shared_ptr<JokerStone> jokerTime, POINT position)
		{
			if (!jokerTime->m_jokerInfo.coolTime)
			{
				...
				boardManager.m_playerInfo.incBlackCount(randomIndex);
				std::cout << "Black Stone : " << boardManager.m_playerInfo.GetBlackCount() << std::endl;


				jokerTime->m_jokerInfo.coolTime = m_jokerInfoMap.find(StoneAbility::jokerTime)->second.coolTime;
			}
...
bool BoardManager::PlaceStone(POINT selectedPosition, StoneType stoneType, StoneAbility stoneAbility)
{
	...
	if (stoneType == StoneType::White)
	{
		...
		{
			m_board[selectedPosition.x][selectedPosition.y] = make_shared<JokerStone>
				(
					BoardToScreenPosition(selectedPosition),
					m_cell, m_stoneOffset,
					stoneAbility,
					m_jokerInfoMap[stoneAbility].jokerType
				);
...
void BoardManager::InitializeJokerInfoMap()
{
	...
	//------------------------------------------------------------------------------------------------ 일반 (set 1)
	m_jokerInfoMap[StoneAbility::jokerOmok]			= { "T_jokerOmok.png", "T_JokerOmok_Tooltip.png", "jokerTeleport.wav",			JokerType::Default, 1, 0, 0, 0, 5, 2, 12, 3, true, true, StoneType::Black };
	m_jokerInfoMap[StoneAbility::jokerSamok]		= { "T_jokerSamok.png", "T_JokerSamok_Tooltip.png", "jokerTeleport.wav",		JokerType::Default, 0, 0, 0, 0, 5, 0, 9, 3, false };
	m_jokerInfoMap[StoneAbility::jokerSammok]		= { "T_jokerSammok.png", "T_JokerSammok_Tooltip.png", "jokerTeleport.wav",		JokerType::Default, 0, 0, 0, 0, 5, 2, 6, 3, false };

	//------------------------------------------------------------------------------------------------ 야생 (set 2)
	m_jokerInfoMap[StoneAbility::jokerEvolution]	= { "T_jokerEvolution.png", "T_JokerEvolution_Tooltip.png", "jokerTeleport.wav",JokerType::Wild, 0, 0, 0, 0, 2, 0, 21, 3, false };						// cost 0
	m_jokerInfoMap[StoneAbility::jokerDansu]		= { "T_jokerDansu.png", "T_JokerDansu_Tooltip.png", "jokerTeleport.wav",		JokerType::Wild, 1, 1, 0, 0, 4, 0, 15, 1, true };
	m_jokerInfoMap[StoneAbility::jokerEgg]			= { "T_jokerEgg.png",	"T_JokerEgg_Tooltip.png", "jokerTeleport.wav",			JokerType::Wild, 5, 0, 0, 1, 5, 0, 15, 1, true };
	m_jokerInfoMap[StoneAbility::jokerOstrichEgg]	= { "T_jokerOstrichEgg.png", "T_JokerEgg_Tooltip.png", "jokerTeleport.wav",		JokerType::Wild, 4, 0, 0, 1, 0, 0, 0, 2, true, false };
	m_jokerInfoMap[StoneAbility::jokerPeacock]		= { "T_jokerPeacock.png",	"T_JokerPeacock_Tooltip.png", "jokerTeleport.wav",	JokerType::Wild, 1, 1, 0, 3, 3, 1, 27, 3, true, true, StoneType::Black };				// cost 4

	//------------------------------------------------------------------------------------------------ 우주 (set 3)
	m_jokerInfoMap[StoneAbility::jokerTeleport]		= { "T_jokerTeleport.png", "T_JokerTeleport_Tooltip.png", "jokerTeleport.wav",	JokerType::Space, 1, 1, 0, 0, 2, 0, 12, 2, true };
	m_jokerInfoMap[StoneAbility::jokerExplode]		= { "T_jokerExplode.png",	"T_JokerExplode_Tooltip.png", "jokerTeleport.wav",	JokerType::Space, 1, 1, 0, 1, 3, 0, 18, 3, true };
	m_jokerInfoMap[StoneAbility::jokerMagnetic]		= { "T_jokerMagnetic.png", "T_JokerMagnetic_Tooltip.png", "jokerTeleport.wav",	JokerType::Space, 1, 1, 0, 3, 4, 2, 15, 2, true };
	m_jokerInfoMap[StoneAbility::jokerBlackhole]	= { "T_Balckhole.png", "T_JokerBlackhole_Tooltip.png", "jokerTeleport.wav",		JokerType::Space, 1, 1, 0, 5, 10, 0, 24, 3, true };

	//------------------------------------------------------------------------------------------------ 단청 (set 4)
	m_jokerInfoMap[StoneAbility::jokerFusion]		= { "T_jokerFusion.png", "T_JokerFusion_Tooltip.png", "jokerTeleport.wav",		JokerType::Dancheong, 0, 0, 0, 1, 2, 0, 15, 2, true, true, StoneType::White };
	m_jokerInfoMap[StoneAbility::jokerTriunion]		= { "T_jokerTriunion.png", "T_JokerFusion_Tooltip.png", "jokerTeleport.wav",	JokerType::Dancheong, 0, 0, 0, 2, 0, 0, 0, 2, true, false, StoneType::White };
	m_jokerInfoMap[StoneAbility::jokerQuadunion]	= { "T_jokerQuadunion.png", "T_JokerFusion_Tooltip.png", "jokerTeleport.wav",	JokerType::Dancheong, 0, 0, 0, 3, 0, 0, 0, 3, true, false, StoneType::White };

	//------------------------------------------------------------------------------------------------ 할로윈 (set 6)
	m_jokerInfoMap[StoneAbility::jokerSplit]		= { "T_jokerSplit.png", "T_JokerSplit_Tooltip.png", "jokerTeleport.wav",		JokerType::Halloween, 3, 0, 0, 20, 2, 0, 15, 2, true };
	m_jokerInfoMap[StoneAbility::jokerWaxseal]		= { "T_jokerWaxseal.png",	"T_JokerWaxseal_Tooltip.png", "jokerTeleport.wav",	JokerType::Halloween, 0, 0, 0, 1, 0, 0, 9, 2, true, true, StoneType::Black };
	m_jokerInfoMap[StoneAbility::jokerFlip]			= { "T_jokerFlip.png", "T_JokerFlip_Tooltip.png", "jokerTeleport.wav",			JokerType::Halloween, 1, 1, 0, 0, 3, 1, 18, 2, true, true, StoneType::Black };
	m_jokerInfoMap[StoneAbility::jokerOthello]		= { "T_jokerOthello.png", "T_JokerOthello_Tooltip.png", "jokerTeleport.wav",	JokerType::Halloween, 1, 0, 0, 2, 2, 4, 15, 1, true, true, StoneType::Black };
	m_jokerInfoMap[StoneAbility::jokerMrchan]		= { "T_jokerMrchan.png", "T_JokerMrchan_Tooltip.png", "jokerTeleport.wav",		JokerType::Halloween, 1, 1, 0, 0, 5, 0, 36, 3, true };

	//------------------------------------------------------------------------------------------------ 자연 (set 7)
	m_jokerInfoMap[StoneAbility::jokerShadow]		= { "T_jokerShadow.png", "T_JokerShadow_Tooltip.png", "jokerTeleport.wav",		JokerType::Natural, 1, 1, 0, 2, 1, 0, 12, 2, true };
	m_jokerInfoMap[StoneAbility::jokerLight]		= { "T_jokerLight.png", "T_JokerLight_Tooltip.png", "jokerTeleport.wav",		JokerType::Natural, 1, 1, 0, 0, 0, 0, 12, 2, true, true, StoneType::Black };
	m_jokerInfoMap[StoneAbility::jokerTime]			= { "T_jokerTime.png", "T_JokerTime_Tooltip.png", "jokerTeleport.wav",			JokerType::Natural, 5, 0, 0, 2, 5, 2, 21, 3, true };
	m_jokerInfoMap[StoneAbility::jokerWind]			= { "T_jokerWind.png", "T_JokerWind_Tooltip.png", "jokerTeleport.wav",			JokerType::Natural, 2, 5, 0, 2, 4, 1, 12, 2, true };
	...
	m_jokerAnimationMap[StoneAbility::jokerMagnetic] = "jokerMagnetic.json";

	m_jokerAnimationMap[StoneAbility::jokerFusion] = "jokerFusion.json";
	m_jokerAnimationMap[StoneAbility::jokerTriunion] = "jokerTriunion.json";
	m_jokerAnimationMap[StoneAbility::jokerQuadunion] = "jokerQuadunion.json";

	m_jokerAnimationMap[StoneAbility::jokerSplit] = "jokerSplit.json";
	m_jokerAnimationMap[StoneAbility::jokerFlip] = "jokerFlip.json";

	m_jokerAnimationMap[StoneAbility::jokerShadow] = "jokerShadow.json";
	m_jokerAnimationMap[StoneAbility::jokerLight] = "jokerLight.json";
	m_jokerAnimationMap[StoneAbility::jokerTime] = "jokerTime.json";
	m_jokerAnimationMap[StoneAbility::jokerWind] = "jokerWind.json";
}
